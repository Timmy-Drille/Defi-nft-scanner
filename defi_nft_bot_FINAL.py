...
# Store the application globally for the scanner task
app_instance = None
...
async def scheduled_scan():
    """Scheduled hourly scan - runs independently"""
    while True:
        try:
            ...
            if projects and USER_CHAT_ID and app_instance:
                for project in projects:
                    summary = format_quick_summary(project)
                    keyboard = [[InlineKeyboardButton("📄 Full Details", callback_data=f"details_{project['id']}")]]
                    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def start_scheduler(application: Application):
    ...
    # Create the scheduled scan task
    asyncio.create_task(scheduled_scan())
    logger.info("Scheduler started - hourly scans enabled")
...
def main():
    ...
    # Run the application with the scheduler
    ...
    # Start scheduler after bot initialization
